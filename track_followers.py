...
def fetch_followers(api: GhApi, username: str) -> list[str]:
    ...
    try:
        ...
        if "404" in error_msg:
            ...
            fatal(
                "GitHub API requires authentication. Please set GH_TOKEN or GITHUB_TOKEN environment variable."
            )
        else:
...
def update_changelog(
    ...
    if changelog_path.exists():
        ...
        if match:
            ...
            new_content = (
                content[: match.start()] + new_entry + content[match.start() :]
            )
        else:
...
def main() -> None:
    ...
    if not validate_username(github_username):
        fatal(
            "Invalid GitHub username format. Username must contain only alphanumeric characters and hyphens."
        )
    ...
    if prev_file.exists():
        ...
        logger.info(
            "No previous day's file found. This is the first run or first day of tracking."
        )
